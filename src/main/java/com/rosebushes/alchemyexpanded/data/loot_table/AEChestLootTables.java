...
public class AEChestLootTables implements LootTableSubProvider {
    ...
    public void generate(BiConsumer<ResourceLocation, LootTable.Builder> biConsumer) {
        biConsumer.accept(AELootTables.AMMO_LAND_LOOT_INJECT, LootTable.lootTable().withPool(LootPool.lootPool().name("alchemyexpanded").setRolls(UniformGenerator.between(0.0F, 6.0F)).add(LootItem.lootTableItem((ItemLike) AEItems.AMMO.get()).setWeight(14).setQuality(1).apply(this.countRange(0.0F, 4.0F))).add(LootItem.lootTableItem((ItemLike) AEItems.FLARE.get()).setWeight(8).setQuality(1).apply(this.countRange(0.0F, 2.0F))).add(LootItem.lootTableItem((ItemLike) AEItems.LASER_AMMO.get()).setWeight(8).setQuality(1).apply(this.countRange(0.0F, 4.0F))).add(LootItem.lootTableItem((ItemLike) AEItems.DITO.get()).setWeight(10).setQuality(1).apply(this.countRange(0.0F, 3.0F))).add(LootItem.lootTableItem((ItemLike) AEItems.GREEN_SUN_BOOK.get()).setWeight(1).setQuality(1).apply(this.countRange(0.0F, 1.0F)))));
    }
